...
class NewsListingViewModelTest {
    ...
    // TODO something wrong with server error and api failed
    ...
    fun `test api failed when api return data first time and force refresh`() {
        ...
        Mockito.`when`(remoteDataSource.getTeamsList())
                .then { ResponseStatus.ApiFailed(500, ErrorResponse(message = "Shit Server Failed :( !!")) }
        ...
        val progressRefreshActualValue = sourceListingViewModel.refreshProgress.value
        val progressRefreshExpectedValue = GONE
//        Truth.assertThat(progressRefreshActualValue).isEqualTo(progressRefreshExpectedValue)

        val errorMessageActualValue = sourceListingViewModel.errorMessage.value
        val errorMessageExpectedValue = ErrorModel(serverMessage = "Shit Server Failed :( !!")
//        Truth.assertThat(errorMessageActualValue).isEqualTo(errorMessageExpectedValue)
